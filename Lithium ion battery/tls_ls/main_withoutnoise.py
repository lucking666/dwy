...
# data_all=addnoise(data_all)
data = data_all[['F2', 'F3', 'F5', 'F6', 'F9', 'cyclelife']].values
print(data)
...
for j in np.arange(2, 7.1, 0.5):
    ...
    for p in range(200):  # 前面的70%产生100组随机的排列
        ...
        X_train = data_train_final[:, 0:-1]
        y_train = np.log10(data_train_final[:, -1]).reshape(-1, 1)

        # 测试集  （这里的测试集并没有进行随机排序，可以放外面或者改成随机排序后的）
        data_test1 = copy.deepcopy(data_test)
        X_test = data_test1[:, 0:-1]
        y_test = np.log10(data_test1[:, -1]).reshape(-1, 1)  # 循环寿命对数值
        y_test_re = data_test1[:, -1].reshape(-1, 1)  # 循环寿命

        # 训练
        W, b = tls(X_train, y_train)
        W_ls, b_ls = ls(X_train, y_train)
        # 测试
        ...
        # LS与TLS预测值对数还原
        a = y_pred_tls.shape[0]  # 测试集大小
        Y_pred_ls_re = []
        Y_pred_tls_re = []
        for i in range(a):
            y_pred_ls_re = math.pow(10, y_pred_ls[i])  # 单个值还原，底数为10
            Y_pred_ls_re = np.append(Y_pred_ls_re, y_pred_ls_re)
            y_pred_tls_re = math.pow(10, y_pred_tls[i])
            Y_pred_tls_re = np.append(Y_pred_tls_re, y_pred_tls_re)
        Y_pred_ls_re = np.array(Y_pred_ls_re).reshape(-1, 1)
        Y_pred_tls_re = np.array(Y_pred_tls_re).reshape(-1, 1)
    ...
    # 还原后的rmse和mape
    all_TLS_rmse.append(np.median(TLS_rmse))
    all_LS_rmse.append(np.median(LS_rmse))
    all_TLS_mape.append(np.median(TLS_mape))
    all_LS_mape.append(np.median(LS_mape))
...
plt.title("doctor——数据集1_还原前RMSE", fontsize=14)
plt.show()
