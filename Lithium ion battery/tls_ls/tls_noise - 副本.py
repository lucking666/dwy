...
for j in range(n):  # 调整噪声大小
    ...
    for p in range(s): # 分割数据
        ...
        for k in range(m):  # 生成m次噪声
            ...
            # noise_X = random.normal(loc=1, scale=2, size=(2, 3))
            # noise_X = np.random.randn(X_train.shape[0], 6) # 对于X，先生成一个噪声矩阵,标准正态分布
            ...
            # 总体最小二乘
            W_tls, b_tls, = tls(X_train.iloc[:, 0:5].values, np.array(X_train.iloc[:,5].values).reshape(-1,1))
            y_pred_tls = np.dot(x_test, W_tls) + b_tls
            tls_rmse.append(rmse(Y_test, y_pred_tls))

            # 最小二乘
            W_ls, b_ls, = ls(X_train.iloc[:, 0:5].values, np.array(X_train.iloc[:,5].values).reshape(-1,1))
